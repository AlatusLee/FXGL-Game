package com.alatus.game.factory;
...
//                要想实现组件之间的自动注入,必须在TankComponent之前已经存在了这个组件,不然他识别不到
    ...
    public Entity newHero(SpawnData data){
        ...
        for (int i = 0; i < 10; i++) {
            images.add(FXGL.image(String.format("Hero/image"+i+".png")));
        }
        AnimationChannel animationChannel = new AnimationChannel(images, Duration.seconds(1));
        AnimatedTexture animatedTexture = new AnimatedTexture(animationChannel);
        animatedTexture.loop();
        return FXGL.entityBuilder(data)
                .view(animatedTexture)
    ...
    @Spawns("HeroWithWing")
    public Entity newHeroWithWing(SpawnData data){
        return FXGL.entityBuilder(data)
